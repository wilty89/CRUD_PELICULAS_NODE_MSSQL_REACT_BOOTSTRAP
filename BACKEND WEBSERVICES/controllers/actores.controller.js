...
const anadirActores = async (req, res) => {
  try {
    const { NombreActores, FechaNacimiento, SexoActores } =
      req.body;
      const FotoActores1 = req.file.filename;
    const pool = await getconection();
    await pool
      .request()
      ...
      .input("FotoActores", sql.VarChar, FotoActores1)
      .query(querys.anadirActores);

    console.log(NombreActores, FechaNacimiento, SexoActores, FotoActores1);
    res.json({ NombreActores, FechaNacimiento, SexoActores, FotoActores1 });
  } catch (error) {
    res.status(500);
    res.send(error.message);
  }
...
const actualizarActores = async (req, res) => {
  try {
    const { NombreActores, FechaNacimiento, SexoActores } =
      req.body;
      let FotoActores = req.file.filename;
    const { ID } = req.params;
    const pool = await getconection();
    await pool
      .request()
     ...
     // console.log(ID, NombreActores, FechaNacimiento, SexoActores, FotoActores)
